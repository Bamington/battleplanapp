...
// Custom Imperium layout with model name and game on left, other info on right
const renderImperiumLayout = async (context: ThemeRenderContext): Promise<void> => {
  const { 
    ctx, 
    canvas, 
    model, 
  ...
  // Debug logging
  console.log('Imperium container bounds:', { minX, maxX, minY, maxY, textElements: textElements.length })
  ...
  ctx.fillStyle = context.customBorderColor || '#F15C22' // Use custom color or fallback to Imperium orange
  ...
  if (showGameDetails && gameName) {
    ...
    if (gameIcon) {
      ...
      try {
        ...
        iconImg.onerror = () => console.warn('Imperium game icon failed to load:', gameIcon)
        iconImg.src = gameIcon
      } catch (error) {
        console.warn('Error loading Imperium game icon:', error)
      }
    }
  }
...
export const imperium: Theme = {
  ...
  renderOptions: {
    renderStandardLayout: renderImperiumLayout,
  ...
  metadata: {
    description: 'Imperium theme with bold Conduit ITC typography and uppercase model names',
    tags: ['imperium', 'tactical', 'custom-font', 'uppercase']
  }
}